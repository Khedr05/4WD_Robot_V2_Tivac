...
static ptrf_cb_t  ptrf_gs_array[GPT_INVALID_TIMER_SELECT]         = {PTR_NULL};
    ...
    /*check on selected timer range*/
        ...
        if(ptr_GPT_configs->enu_GPT_mode < GPT_INVALID_MODE)
               ...
               if (ptr_GPT_configs->bool_use_interrupt == TRUE)
							 {
								 if(ptr_GPT_configs->ptrf_call_back == PTR_NULL)
								 {
               ...
               bool_gs_is_timer_init[enu_loc_selected_timer] = TRUE;							 
					...
					{
						/*TODO support GPT_INPUT_EDGE_TIME_MODE */
					}
					else if(ptr_GPT_configs->enu_GPT_mode == GPT_PWM_MODE)
					{
						/*TODO support GPT_PWM_MODE */
					}
					else
					{
						/* do nothing*/
...
{
	enu_GPT_status_t enu_GPT_status_return = GPT_OKAY;
  double f64_tick_time   = ONE_F/SystemCoreClock;
	...
	if(enu_arg_GPT_timer_select < GPT_INVALID_TIMER_SELECT)
	{
		...
		if(bool_gs_is_timer_init[enu_arg_GPT_timer_select] == TRUE)
		{
			if(enu_arg_time_unit < TIME_UNIT_INVALID)
			{
				...
				GPTMTAILR(enu_arg_GPT_timer_select) = (Uint32_t)f64_total_ticks;
...
enu_GPT_status_t GPT_get_elapsed_time(enu_GPT_timer_select_t enu_arg_GPT_timer_select , Uint32_t *u32_ptr_time_ms)
{
	...
	if(enu_arg_GPT_timer_select < GPT_INVALID_TIMER_SELECT)
	{
		*u32_ptr_time_ms = (Uint32_t)((GPTMTAILR(enu_arg_GPT_timer_select) - GPTMTAR(enu_arg_GPT_timer_select))/(CLK_FREQ*MILLI_CONVERTER));
	}
...
enu_GPT_status_t GPT_get_remaining_time(enu_GPT_timer_select_t enu_arg_GPT_timer_select , Uint32_t *u32_ptr_time_ms)
{
	...
	if(enu_arg_GPT_timer_select < GPT_INVALID_TIMER_SELECT)
	{
		*u32_ptr_time_ms = (Uint32_t)(GPTMTAR(enu_arg_GPT_timer_select)/(CLK_FREQ*MILLI_CONVERTER));
	}
...
enu_GPT_status_t GPT_enable_interrupt(enu_GPT_timer_select_t enu_arg_GPT_timer_select)
{
	...
	if(enu_arg_GPT_timer_select < GPT_INVALID_TIMER_SELECT)
	{
		if (bool_gs_is_timer_init[enu_arg_GPT_timer_select] == TRUE)
		{
				...
				else if(enu_arg_GPT_timer_select == GPT_WIDE_TIMER1_SELECT)
				{
...
void TIMER0A_Handler(void)
{
	if(ptrf_gs_array[GPT_TIMER0_SELECT] != PTR_NULL)
	{
...
void WTIMER0A_Handler(void)
{
	if(ptrf_gs_array[GPT_WIDE_TIMER0_SELECT] != PTR_NULL)
	{
...
{
	if(ptrf_gs_array[GPT_WIDE_TIMER1_SELECT] != PTR_NULL)
	{
		ptrf_gs_array[GPT_WIDE_TIMER1_SELECT]();
		SET_BIT(GPTMICR(GPT_WIDE_TIMER1_SELECT),GPTMICR_TATOCINT);
	}
	else
	{
		//do nothig
